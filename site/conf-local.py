...
"""
Nikola local configuration file for www.vicchi.org
"""
...
def emojify_filter(value: str) -> str:
    """
    Jinja2 custom filter to add CLDR short form emoji
    """

    return emoji.emojize(f':{value}:', language='alias')

def dateparser_filter(datestamp: str, datefmt: str) -> str:
    """
...
BLOG_AUTHOR = "Gary Gale"  # (translatable)
...
# those.
TEMPLATE_FILTERS = {
    'dateparser': dateparser_filter,
    'emojify': emojify_filter
}
...
GLOBAL_CONTEXT = {
    ...
    'indieweb': {
        'metadata': {
            ...
            'url': 'http://hopper.vicchi.xyz:8888/metadata'
        },
        'authorization': {
            'rel': 'authorization_endpoint',
            'url': 'http://hopper.vicchi.xyz:8888/auth'
        },
        'token': {
            'rel': 'token_endpoint',
            'url': 'http://hopper.vicchi.xyz:8888/token'
        }
    }
}
